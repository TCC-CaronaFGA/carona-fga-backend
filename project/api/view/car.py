...
def car_list(resp):
    ...
    print(cars_json, flush=True)

    response_object = createSuccessMessage('car was created')
...
def car_registration(resp):
    ...
    if CarModel.find_by_plate(plate):
        return jsonify(createFailMessage('{} already exists'.format(plate))), 400
...
def car_update(resp, idCar):
    ...
    if car.idUser != user['idUser']:
        return jsonify(createFailMessage('{} does not belongs to you'.format(plate))), 400
    ...
    try:
        ...
        db.session.commit()
        response_object = createSuccessMessage('car was updated')
        response_object.update(car.to_json())
        return jsonify(response_object), 200
    except Exception as e:
        db.session.rollback()
...
def car_delte(resp, idCar):
    ...
    if car is None:
        return jsonify(createFailMessage('{} does not exists'.format(idCar))), 404


    if car.idUser != user['idUser']:
        return jsonify(createFailMessage('{} does not belongs to you'.format(idCar))), 400

    try:
        car.delete_from_db()
        return jsonify(createSuccessMessage('car was deleted')), 200
    except Exception as e:
        db.session.rollback()
        return jsonify(createFailMessage(e.message)), 503
